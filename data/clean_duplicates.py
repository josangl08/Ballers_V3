...
def find_duplicate_groups():
    ...
    duplicates_query = (
        ...
        .group_by(Session.coach_id, Session.player_id, func.date(Session.start_time))
...
def clean_duplicates_smart():
    ...
    for group in duplicate_groups:
        ...
        print(
            f"   📅 Coach {group.coach_id} + Player {group.player_id} en {group.session_date}: {len(sessions_in_group)} duplicados"
        )
        ...
        print(
            f"     ✅ Conservando sesión #{session_to_keep.id} (score: {session_score(session_to_keep):.0f})"
        )

        # Eliminar las duplicadas
        for session in sessions_to_delete:
            print(
                f"     🗑️ Eliminando sesión #{session.id} (score: {session_score(session):.0f})"
            )
            db.delete(session)
            total_deleted += 1
    ...
    response = input(
        f"\n¿Confirmar eliminación de {total_deleted} sesiones duplicadas? (s/N): "
    ).lower()
    if response == "s":
        db.commit()
