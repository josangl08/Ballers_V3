...
class FuturePDIPredictor:
    ...
    def create_training_dataset(
        ...
        """
        logger.info(f"🔧 Creando dataset temporal sin data leakage (predecir +{years_ahead} años)")
        ...
        logger.info(f"📊 Datos: {len(historical_df)} registros, temporadas: {sorted(historical_df['season_year'].unique())}")
        ...
        for target_year in available_years[years_ahead:]:  # Solo años donde podemos tener historical data
            train_years = [y for y in available_years if y < target_year][-3:]  # Últimas 3 temporadas como máximo
            
            if len(train_years) == 0:
                continue
                
            logger.info(f"🎯 Prediciendo temporada {target_year} usando temporadas {train_years}")
            ...
            common_players = set(train_data["player_id"]).intersection(set(target_data["player_id"]))
            
            logger.info(f"   📋 Jugadores comunes: {len(common_players)}")
            
            for player_id in common_players:
                player_train = train_data[train_data["player_id"] == player_id].sort_values("season_year")
    ...
    def _engineer_temporal_features(self, player_train: pd.DataFrame, target_year: int) -> dict:
        """
        ...
        try:
            ...
            basic_features = [
                'Age', 'Height', 'Weight', 'Minutes played', 'Matches played',
                'Pass accuracy, %', 'Duels won, %', 'Successful dribbles, %',
                'Goals per 90', 'Assists per 90', 'xG per 90', 'xA per 90',
                'Defensive duels won, %', 'Aerial duels won, %', 'Yellow cards per 90'
            ]
            
            for feature in basic_features:
                if feature in latest_season.index and pd.notna(latest_season[feature]):
                    features[f'{feature.replace(" ", "_").replace(",", "").replace("%", "pct")}'] = float(latest_season[feature])
                else:
                    features[f'{feature.replace(" ", "_").replace(",", "").replace("%", "pct")}'] = 0.0
            ...
            if len(player_train) >= 2:
                ...
                if 'Minutes played' in recent_seasons.columns:
                    ...
                    if len(minutes_vals) >= 2:
                        features['minutes_trend'] = float(minutes_vals.pct_change().fillna(0).mean())
                        features['minutes_mean'] = float(minutes_vals.mean())
                        features['minutes_std'] = float(minutes_vals.std()) if len(minutes_vals) > 1 else 0.0
                
                # Tendencia de rendimiento
                if 'Pass accuracy, %' in recent_seasons.columns:
                    pass_vals = recent_seasons['Pass accuracy, %'].fillna(0)
                    if len(pass_vals) >= 2:
                        features['pass_acc_trend'] = float(pass_vals.pct_change().fillna(0).mean())
                        features['pass_acc_consistency'] = float(1.0 / (1.0 + pass_vals.std())) if pass_vals.std() > 0 else 1.0
            ...
            features['years_to_target'] = float(target_year - latest_season.get('season_year', target_year-1))
            
            # Estabilidad del jugador
            if len(player_train) >= 2:
                team_changes = len(player_train['Team'].dropna().unique()) if 'Team' in player_train.columns else 1
            ...
            if 'PDI' in latest_season.index and pd.notna(latest_season['PDI']):
                ...
                for metric in ['Pass accuracy, %', 'Duels won, %', 'Minutes played']:
                    if metric in latest_season.index and pd.notna(latest_season[metric]):
