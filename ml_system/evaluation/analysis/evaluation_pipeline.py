...
class StatisticalAnalyzer:
        ...
        try:
            ...
            return {
                ...
                "interpretation": self._interpret_ttest_results(
                    performance1.model_name,
                    performance2.model_name,
                    metric,
                    p_value,
                    effect_size,
                ),
    ...
    def compare_multiple_models_friedman(
        self, performances: List[ModelPerformance], metric: str = "mae"
    ) -> Dict[str, Any]:
        """
            ...
            if is_significant:
                for i in range(len(model_names)):
                    for j in range(i + 1, len(model_names)):
                        ...
                        pairwise_comparisons[
                            f"{model_names[i]}_vs_{model_names[j]}"
                        ] = comparison
    ...
    def _interpret_ttest_results(
        self, model1: str, model2: str, metric: str, p_value: float, effect_size: float
    ) -> str:
        """Interpreta resultados de t-test académicamente."""

        significance = (
            "significativa" if p_value < self.significance_level else "no significativa"
        )
    ...
    def _interpret_friedman_results(
        self, models: List[str], metric: str, p_value: float, is_significant: bool
    ) -> str:
...
class AcademicVisualizer:
    ...
    def plot_model_comparison_academic(
        self, performances: List[ModelPerformance], config: EvaluationConfig
    ) -> None:
        """
    ...
    def plot_position_analysis(self, performances: List[ModelPerformance]) -> None:
        ...
        models = (
            list(performances[0].position_performance.get(positions[0], {}).keys())
            if positions
            else []
        )
...
class EvaluationPipeline:
    ...
    def __init__(self, config: EvaluationConfig = None):
        ...
        logger.info(
            f"CV folds: {self.config.cv_folds}, Métrica principal: {self.config.primary_metric}"
        )
            ...
            try:
                ...
                if importance_values is not None and feature_names is not None:
                    ...
                    if np.sum(importance_values) > 0:
                        importance_values = importance_values / np.sum(
                            importance_values
                        )
    ...
    def evaluate_multiple_models(
        ...
        for model_name, model in models.items():
            ...
            performance = self.evaluate_single_model(
                model=model, X=X, y=y, positions=positions, feature_names=feature_names
            )
        ...
        self.visualizer.plot_model_comparison_academic(
            list(performances.values()), self.config
        )
    ...
    def _analyze_by_position(
        self, predictions: np.ndarray, actuals: np.ndarray, positions: np.ndarray
    ...
    def _perform_statistical_analysis(
        ...
        if len(performances) >= 3:
            friedman_result = (
                self.statistical_analyzer.compare_multiple_models_friedman(
            ...
            print(
                f"   Significativo: {'✅ Sí' if friedman_result.get('is_significant', False) else '❌ No'}"
            )
        ...
        for i in range(len(performances)):
            for j in range(i + 1, len(performances)):
                ...
                if "error" not in comparison:
                    ...
                    print(
                        f"      Significativo: {'✅ Sí' if comparison['is_significant'] else '❌ No'}"
                    )
                    print(f"      Tamaño del efecto: {comparison['effect_size']:.3f}")

                    ci_lower, ci_upper = comparison.get(
    ...
    def _save_evaluation_results(
        ...
        try:
            ...
            csv_path = (
                Path(self.config.results_dir) / f"model_comparison_{timestamp}.csv"
            )
            ...
            json_path = (
                Path(self.config.results_dir) / f"detailed_results_{timestamp}.json"
            )
    ...
    def generate_academic_report(
        ...
        report.append(
            f"   • Validación cruzada: {self.config.cv_folds}-fold {self.config.cv_strategy}"
        )
        report.append(f"   • Métrica principal: {self.config.primary_metric.upper()}")
        report.append(
            f"   • Nivel de confianza: {self.config.confidence_level*100:.0f}%"
        )
        ...
        for name, perf in performances.items():
            report.append(
                f"   • {name}: {perf.n_samples} muestras, {perf.n_features} features"
            )
        ...
        report.append(
            f"   • MAE: {best_perf.mae:.3f} ± {best_perf.cv_std.get('mae', 0):.3f}"
        )
        report.append(f"   • RMSE: {best_perf.rmse:.3f}")
        report.append(
            f"   • R²: {best_perf.r2:.3f} ± {best_perf.cv_std.get('r2', 0):.3f}"
        )
        ...
        models_meeting_target = [
            (name, perf) for name, perf in performances.items() if perf.mae < target_mae
        ]

        report.append(f"\n🎯 OBJETIVO ACADÉMICO (MAE < {target_mae}):")
        if models_meeting_target:
            report.append(
                f"   ✅ {len(models_meeting_target)} modelo(s) cumplen objetivo:"
            )
        ...
        if len(models_meeting_target) > 0:
            ...
            report.append(
                f"   🤖 Evaluar modelos más complejos (RF, Gradient Boosting)"
            )

        # Validez estadística
        cv_stability = 1 - (
            best_perf.cv_std.get("mae", 0) / best_perf.cv_mean.get("mae", 1)
        )
        report.append(f"\n📈 VALIDEZ ESTADÍSTICA:")
