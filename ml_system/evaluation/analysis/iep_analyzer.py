...
class IEPAnalyzer:
    ...
    def analyze_player_efficiency_tier(
        ...
        try:
            ...
            logger.info(f"✅ Análisis IEP completado - Score: {enhanced_results['iep_metrics']['iep_score']}, Tier: {enhanced_results['iep_metrics']['cluster_tier']}")
    ...
    def get_position_cluster_analysis(
        ...
        try:
            ...
            if use_cache and cache_key in self._cluster_cache:
                cache_age = datetime.now() - self._last_cache_update.get(cache_key, datetime.min)
            ...
            cluster_results = self.iep_calculator.calculate_position_clusters(position, season)
            ...
            logger.info(f"✅ Análisis clustering completado - {enhanced_results['data_quality']['total_players']} jugadores, {enhanced_results['clustering_results']['n_clusters']} clusters")
    ...
    def generate_league_efficiency_benchmarks(
        ...
        try:
            ...
            if not positions:
                positions = ['GK', 'CB', 'LB', 'RB', 'DMF', 'CMF', 'AMF', 'LW', 'RW', 'CF']
            ...
            for position in positions:
                ...
                if 'error' in cluster_analysis:
                    logger.warning(f"Saltando {position} - error: {cluster_analysis['error']}")
                    continue
                ...
                position_benchmarks = {
                    ...
                    'silhouette_score': cluster_analysis['clustering_results']['silhouette_score'],
                    'variance_explained': cluster_analysis['pca_analysis']['total_variance_explained'],
                    'tier_distribution': {},
                    'performance_ranges': {}
                }
            ...
            benchmarks['summary_stats'] = {
                ...
                'total_positions': len([p for p in positions if p in benchmarks['league_benchmarks']]),
                'average_clusters_per_position': round(all_clusters_count / max(1, len(benchmarks['league_benchmarks'])), 1),
            ...
            logger.info(f"✅ Benchmarks generados - {all_players_count} jugadores, {len(benchmarks['league_benchmarks'])} posiciones")
    ...
    def get_available_positions_for_season(self, season: str = "2024-25") -> List[str]:
        ...
        try:
            ...
            standard_positions = ['GK', 'CB', 'LB', 'RB', 'DMF', 'CMF', 'AMF', 'LW', 'RW', 'CF']
            
            for position in standard_positions:
                # Verificar datos sin ejecutar clustering completo
                position_data = self.iep_calculator._get_position_data(position, season, min_matches=5)
                
                if len(position_data) >= 10:  # Mínimo para clustering
                    available_positions.append(position)
            
            logger.info(f"📊 Posiciones disponibles para {season}: {available_positions}")
    ...
    def _enhance_player_analysis(self, iep_results: Dict) -> Dict:
        ...
        try:
            ...
            enhanced['interpretations'] = {
                ...
                'development_recommendations': self._generate_development_recommendations(enhanced)
            }
    ...
    def _enhance_cluster_analysis(self, cluster_results: Dict) -> Dict:
        ...
        try:
            ...
            enhanced['clustering_insights'] = {
                ...
                'position_characteristics': self._analyze_position_patterns(cluster_results)
            }
    ...
    def _generate_development_recommendations(self, player_data: Dict) -> List[str]:
        ...
        try:
            ...
            if iep_score < 50:
                recommendations.append('Focus on fundamental technical skills improvement')
                ...
                recommendations.append('Work on consistency across different match scenarios')
            else:
            ...
            if key_features.get('pass_accuracy_pct', 0) < 80:
                recommendations.append('Improve passing accuracy through targeted drills')
            
            if key_features.get('duels_won_pct', 0) < 50:
                recommendations.append('Strengthen physical preparation and duel technique')
            
        except Exception as e:
            logger.debug(f"Error generando recomendaciones: {e}")
            recommendations.append('Continuar desarrollo integral según plan de entrenamiento')
    ...
    def _generate_tier_insights(self, cluster_data: Dict) -> List[str]:
        ...
        try:
            ...
            for cluster_id, analysis in cluster_analysis.items():
                ...
                if percentage > 40:
                    insights.append(f'{tier_label} dominates the position ({percentage}% of players)')
                elif percentage < 15:
                    insights.append(f'{tier_label} is rare in this position ({percentage}% of players)')
                else:
    ...
    def _analyze_position_patterns(self, cluster_data: Dict) -> List[str]:
        ...
        try:
            ...
            if position in ['CF', 'RW', 'LW']:
                patterns.append('Offensive position - Goal and assist contribution critical')
            elif position in ['CMF', 'AMF', 'DMF']:
                patterns.append('Midfield position - Passing and creativity key factors')
            elif position in ['CB', 'LB', 'RB']:
                patterns.append('Defensive position - Tackling and clearances essential')
            elif position == 'GK':
                patterns.append('Goalkeeper position - Save rate and distribution crucial')
            
            # Análisis de componentes PCA si disponible
            variance_ratio = cluster_data.get('pca_analysis', {}).get('explained_variance_ratio', [])
    ...
    def _save_league_benchmarks(self, benchmarks: Dict, season: str):
        ...
        try:
            ...
            logger.info(f"💾 Benchmarks liga guardados: {json_filename}, {csv_filename}")
    ...
    def _export_benchmarks_csv(self, benchmarks: Dict, filepath: Path):
        ...
        try:
            ...
            with open(filepath, 'w', newline='', encoding='utf-8') as csvfile:
                ...
                # Header
                writer.writerow([
                    'Position', 'Total_Players', 'Clusters', 'Silhouette_Score', 
                    'Variance_Explained', 'Elite_Count', 'Elite_Avg_Score',
                    'Strong_Count', 'Strong_Avg_Score', 'Average_Count', 'Average_Avg_Score'
                ])
                
                # Data rows
                for position, data in benchmarks['league_benchmarks'].items():
                    row = [position, data['total_players'], data['clusters'], data['silhouette_score']]
                    ...
                    for tier in tiers:
                        ...
                        avg_score = data['performance_ranges'].get(tier, {}).get('mean', 0)
...
logger.info("🎯 Módulo IEPAnalyzer cargado - Interface para análisis de eficiencia posicional")