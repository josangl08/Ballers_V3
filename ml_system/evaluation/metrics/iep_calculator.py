...
class IEPCalculator:
    ...
    def __init__(self, session_factory=None):
        ...
        self.base_features = [
            'goals_per_90', 'assists_per_90', 'pass_accuracy_pct',
            'duels_won_pct', 'shots_per_90', 'interceptions_per_90',
            'tackles_per_90', 'clearances_per_90', 'minutes_played'
        ]
    ...
    def calculate_position_clusters(
        ...
        try:
            ...
            if not position_data or len(position_data) < 10:
                logger.warning(f"Datos insuficientes para clustering {position}: {len(position_data) if position_data else 0} jugadores")
                return {'error': 'insufficient_data', 'player_count': len(position_data) if position_data else 0}
            
            # Preparar features para clustering
            feature_matrix, player_info = self._prepare_features_matrix(position_data, position)
            
            if feature_matrix.shape[0] < 10:
                logger.warning(f"Features insuficientes después de limpieza: {feature_matrix.shape[0]} jugadores")
                return {'error': 'insufficient_features', 'player_count': feature_matrix.shape[0]}
            
            # Normalizar features
            normalized_features = self.scaler.fit_transform(feature_matrix)
            
            # Configurar clustering por posición
            config = self.position_cluster_config.get(position, {'n_clusters': 3, 'features_weight': 'balanced'})
            n_clusters = min(config['n_clusters'], len(position_data) // 3)  # Ajustar por datos disponibles
            ...
            results = {
                ...
                'clustering_results': {
                    ...
                    'cluster_distribution': {int(i): int(np.sum(cluster_labels == i)) for i in range(n_clusters)}
                },
                'pca_analysis': {
                    'explained_variance_ratio': [float(r) for r in self.pca.explained_variance_ratio_],
            ...
            for i, info in enumerate(player_info):
                player_result = {
                    ...
                    'cluster_label': self._get_cluster_label(cluster_labels[i], n_clusters),
                    'iep_score': round(iep_scores[i], 1),
                    'pca_components': [float(pca_components[i, 0]), float(pca_components[i, 1])],
                    'key_features': {
            ...
            logger.info(f"   🎯 Silhouette: {silhouette_avg:.3f}, Varianza PCA: {sum(self.pca.explained_variance_ratio_):.1%}")
    ...
    def calculate_player_iep(
        ...
        try:
            ...
            with self.session_factory() as session:
                player_stats = session.query(ProfessionalStats).filter(
                    ProfessionalStats.player_id == player_id,
                    ProfessionalStats.season == season
                ).first()
                
                if not player_stats:
                    logger.warning(f"No se encontraron datos para jugador {player_id} en {season}")
                    return {'error': 'no_player_data', 'player_id': player_id, 'season': season}
                ...
                if not position:
                    logger.warning(f"No se pudo determinar posición para jugador {player_id}")
            ...
            if 'error' in cluster_results:
                logger.warning(f"Error en clustering posicional: {cluster_results['error']}")
            ...
            for player in cluster_results['players_data']:
                ...
                if str(player_id) in player['player_name'] or player['player_name'] in (player_stats.full_name or ''):
                    player_data = player
                    break
            
            if not player_data:
                logger.warning(f"Jugador {player_id} no encontrado en clustering de {position}")
                return {'error': 'player_not_in_cluster', 'player_id': player_id, 'position': position}
            ...
            individual_result = {
                ...
                'efficiency_components': {
                    ...
                    'explained_variance': cluster_results['pca_analysis']['total_variance_explained']
                },
                'position_context': {
                    'total_players_analyzed': len(cluster_results['players_data']),
                    'cluster_distribution': cluster_results['clustering_results']['cluster_distribution'],
                    'silhouette_score': cluster_results['clustering_results']['silhouette_score']
                },
                'key_performance_features': player_data['key_features']
            }
            
            logger.info(f"✅ IEP individual calculado: Score {player_data['iep_score']}, Tier {player_data['cluster_label']}")
    ...
    def _get_position_data(self, position: str, season: str, min_matches: int) -> List[Dict]:
        ...
        try:
            with self.session_factory() as session:
                ...
                for stat in stats_data:
                    ...
                    clearances_estimate = (
                        (stat.aerial_duels_per_90 or 0) * (stat.aerial_duels_won_pct or 0) / 100 +  # Aéreos ganados
                        (stat.interceptions_per_90 or 0)  # Interceptaciones
                    )
                    ...
                    player_dict = {
                        ...
                        'tackles_per_90': stat.sliding_tackles_per_90 or 0,  # Mapeo: sliding_tackles como proxy para tackles totales
                ...
                logger.info(f"📊 Datos obtenidos para {position}: {len(position_data)} jugadores")
    ...
    def _calculate_iep_scores(
        ...
        try:
            ...
            cluster_quality_order = sorted(range(n_clusters), key=lambda i: cluster_centers[i][0], reverse=True)
            ...
            for i in range(len(normalized_features)):
                ...
                base_score = 85 - (cluster_rank * 20)  # Elite: 85, Strong: 65, Average: 45, Dev: 25
                
                # Ajuste por posición dentro del cluster (±15 puntos)
                pc1_adjustment = min(15, max(-15, pca_pos[0] * 10))  # Principal component influence
                pc2_adjustment = min(5, max(-5, pca_pos[1] * 3))    # Secondary component influence
    ...
    def _analyze_clusters(
        ...
        try:
            ...
            for cluster_id in range(n_clusters):
                ...
                cluster_players = [player_info[i] for i in range(len(player_info)) if cluster_mask[i]]
                ...
                analysis[cluster_id] = {
                    ...
                    'percentage': round(len(cluster_players) / len(player_info) * 100, 1),
                    'top_players': [p['name'] for p in cluster_players[:5]],  # Top 5
                    'performance_profile': cluster_stats,
                    'characteristics': self._get_cluster_characteristics(cluster_stats, position)
                }
    ...
    def _get_cluster_characteristics(self, cluster_stats: Dict, position: str) -> List[str]:
    ...
    def _calculate_percentile(self, player_score: float, all_scores: List[float]) -> float:
...
logger.info("🧮 Módulo IEPCalculator cargado - Sistema no supervisado para análisis de eficiencia posicional")