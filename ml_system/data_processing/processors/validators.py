...
class DataValidators:
        ...
        try:
            ...
            if matches_val > 0:
                ...
                if avg_minutes_per_match > 95:  # Pequeño margen por tiempo añadido
                    return (
                        False,
                        f"Promedio de {avg_minutes_per_match:.1f} min/partido es muy alto",
                    )
    ...
    def validate_goals_shots_consistency(
        ...
        try:
            ...
            if goals_val > shots_val:
                return (
                    False,
                    f"Goles ({goals_val}) no puede exceder tiros ({shots_val})",
                )
    ...
    def validate_dataframe_integrity(
        ...
        for idx, row in df.iterrows():
            ...
            if validation_config.get("validate_consistency", False):
                ...
                if "Matches played" in row and "Minutes played" in row:
                    is_valid, error = (
                        DataValidators.validate_matches_minutes_consistency(
                            row["Matches played"], row["Minutes played"]
                        )
                    )
                    if not is_valid:
                        record_errors.append(
                            f"Inconsistencia partidos-minutos: {error}"
                        )
            ...
            if validation_config.get("validate_ranges", False):
                ...
                for col in percentage_columns:
                    if col in row:
                        ...
                        if not is_valid:
                            record_errors.append(
                                f"Porcentaje inválido en {col}: {error}"
                            )
        ...
        logger.info(
            f"✅ Validación completada: {stats['valid_records']}/{stats['total_records']} válidos"
        )
