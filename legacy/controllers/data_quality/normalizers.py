...
class DataNormalizers:
    ...
    def normalize_currency(value: Any, currency_symbol: str = "€") -> Optional[float]:
        ...
        try:
            if isinstance(value, str):
                ...
                cleaned = (
                    value.replace(currency_symbol, "").replace(",", "").replace(" ", "")
                )
    ...
    def normalize_dataframe_columns(
        df: pd.DataFrame, column_config: dict
    ) -> pd.DataFrame:
        """
        ...
        for column, normalizer_type in column_config.items():
            if (
                column in df_normalized.columns
                and normalizer_type in normalizer_mapping
            ):
