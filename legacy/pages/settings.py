...
from controllers.notification_controller import (auto_cleanup_old_problems,
                                                 clear_sync_problems,
                                                 get_sync_problems)
...
from controllers.user_controller import (UserController, create_user_simple,
                                         delete_user_simple,
...
def create_user_form():
    ...
    with st.container(border=True):
        ...
        with st.form(
            f"create_user_form_{st.session_state.create_user_success_count}",
            ...
            if user_type == "coach":
                ...
                service_type = st.multiselect(
                    "Service type(s)", options=service_options, default=["Basic"]
                )

                col5, col6 = st.columns(2)
                with col5:
                    enrolment = st.number_input(
                        "Number of enrolled sessions", min_value=0, value=0
                    )
                with col6:
                ...
                with col8:
                    permit_level = st.number_input(
                        "Permit Level", min_value=1, max_value=10, value=1
                    )
            ...
            if submit:
                ...
                if user_type == "coach":
                    ...
                    profile_data.update({"role": role, "permit_level": permit_level})
...
def edit_any_user():
    ...
    selected_user_id = st.selectbox(
        ...
        format_func=lambda x: next(
            (
                f"{u['Name']} ({u['Username']}, {u['User Type']})"
                for u in users_data
                if u["ID"] == x
            ),
            "",
    ...
    with st.form(
        f"admin_edit_user_form_{st.session_state.edit_user_success_count}",
        clear_on_submit=False,
    ):
        ...
        if new_user_type == "coach":
            ...
            service_input = st.multiselect(
                "Service(s)", options=service_options, default=current_services
            )
            enrolment_input = st.number_input(
                "Number of enrolled sessions", min_value=0, value=enrolment_current
            )
            ...
            permit_level_input = st.number_input(
                "Permit Level", min_value=1, max_value=10, value=permit_level_current
            )

            profile_data.update(
                {"role": role_input, "permit_level": permit_level_input}
            )
        ...
        new_profile_photo = st.file_uploader(
            "New Profile Picture", type=["jpg", "jpeg", "png"]
        )
        ...
        if submit:
            ...
            update_data = {
                ...
                "new_user_type": (
                    new_user_type if new_user_type != user_data["user_type"] else None
                ),
...
def delete_user():
    ...
    confirm_text = st.text_input(
        ...
        key=f"delete_confirm_{st.session_state.delete_user_success_count}",
    )
...
def manage_user_status():
    ...
    with col2:
        status_filter = st.selectbox(
            "Filter by Status:", options=["All", "Active  🟢", "Inactive  🔴"]
        )
    ...
    selected_user_id = st.selectbox(
        ...
        format_func=lambda x: next(
            (opt[1] for opt in selector_options if opt[0] == x), ""
        ),
    )

    # Encontrar usuario seleccionado
    selected_user = next(
        (u for u in filtered_users if u["ID"] == selected_user_id), None
    )
    ...
    if selected_user:
        ...
        with col1:
            st.info(
                f"**{selected_user['Name']}** - Current status: **{current_status}**"
            )
        ...
        with col2:
            if st.button(f"{action_icon} {action} User", type="primary"):
                ...
                with UserController() as controller:
                    success, message = controller.toggle_user_status(
                        selected_user["ID"]
                    )
...
def system_settings():
    ...
    should_show_details = (
        st.session_state.get("show_sync_details", False)
        or st.session_state.get("show_sync_problems_tab", False)
        or bool(get_sync_problems())
    )
    ...
    if should_show_details:
        with st.container():
            ...
            if result_data:
                ...
                if past_updated > 0:
                    st.info(
                        f"📅 Additionally: {past_updated} past sessions marked as completed"
                    )
                ...
                if len(rejected_events) > 0:
                    st.error(
                        f"🚫 Sync completed with {len(rejected_events)} rejected events"
                    )
                    if total_changes > 0:
                        st.error(
                            f"📊 Changes: {total_changes} total ({imported} imported, {updated} updated, {deleted} deleted)"
                        )
                    if len(warning_events) > 0:
                        st.warning(
                            f"⚠️ Additionally: {len(warning_events)} events with warnings"
                        )
                ...
                if rejected_events or warning_events:
                    if rejected_events and warning_events:
                        detail_tab1, detail_tab2 = st.tabs(
                            [
                                f"🚫 Rejected ({len(rejected_events)})",
                                f"⚠️ Warnings ({len(warning_events)})",
                            ]
                        )
                        ...
                        (detail_tab2,) = st.tabs(
                            [f"⚠️ Events with Warnings ({len(warning_events)})"]
                        )
                        detail_tab1 = None
                ...
                with col2:
                    if st.button(
                        "🧹 Clear sync results", help="Mark all problems as resolved"
                    ):
                        clear_sync_problems()
                        if "last_sync_result" in st.session_state:
        ...
        if status["total_syncs"] > 0:
            ...
            if status.get("last_changes"):
                ...
                if imported + updated + deleted > 0:
                    st.info(
                        f"📊 Latest changes: {imported} imported, {updated} updated, {deleted} deleted"
                    )
...
def show_content():
    ...
    with tab2:  # Users - Pestaña secundaria
        ...
        user_tab1, user_tab2, user_tab3, user_tab4 = st.tabs(
            ["Create User", "Edit User", "Delete User", "User Status"]
        )
