...
def create_pdi_evolution_chart(player_id, seasons=None):
    ...
    try:
        ...
        try:
            ...
            MODEL_MAE = service_info.get('model_mae', 3.692)  # MAE del modelo optimizado
            model_type = service_info.get('model_type', 'optimized')
            logger.info(f"🎯 Usando modelo {model_type} con MAE {MODEL_MAE} para intervalos de confianza")
        except Exception as e:
            logger.warning(f"No se pudo cargar servicio optimizado, usando MAE por defecto: {e}")
