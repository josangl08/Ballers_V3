...
class WebhookServer:
    ...
    def __init__(self, port: int = WEBHOOK_PORT, secret_token: str = WEBHOOK_SECRET_TOKEN):
        self.port = port
    ...
    def _setup_routes(self):
        ...
        def handle_calendar_webhook():
            ...
            try:
                ...
                if resource_state in ['exists', 'updated']:
                    ...
                    logger.info("📞 Webhook sync message received (channel established)")
                else:
        ...
        def webhook_status():
            ...
            return jsonify({
                ...
                "stats": self.stats,
                "uptime_seconds": (
                    time.time() - self.stats.get("server_started_at", time.time())
                ) if self.stats.get("server_started_at") else 0
            }), 200
    ...
    def _process_webhook_async(self, channel_id: str, resource_id: str, resource_state: str):
        """
        ...
        def process():
            try:
                logger.info(f"🔄 Processing webhook: {resource_state} for resource {resource_id}")
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            logger.info(f"📡 Calendar webhook endpoint: http://localhost:{self.port}/webhook/calendar")
