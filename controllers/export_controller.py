...
class ExportController:
    ...
    def generate_player_report(
        ...
        # Título
        title = Paragraph(f"Player Report: {player.user.name}", self.styles['CustomTitle'])
        story.append(title)
        ...
        try:
            if user.profile_photo and os.path.exists(user.profile_photo):
                photo_image = Image(user.profile_photo, width=1.5*inch, height=1.5*inch)
                photo_cell = photo_image
            else:
                photo_cell = Paragraph("No photo available", self.styles['CustomNormal'])
        ...
        personal_info_paragraph = Paragraph(personal_info_text, self.styles['CustomNormal'])
        
        # Crear tabla con foto y datos lado a lado
        personal_table = Table([[photo_cell, personal_info_paragraph]], colWidths=[2*inch, 4*inch])
        ...
        # Calendario simulado
        if sessions:
            story.append(Paragraph(f"Calendar View ({start_date.strftime('%d/%m/%Y')} - {end_date.strftime('%d/%m/%Y')})", self.styles['CustomSubtitle']))
            
            calendar_table = self._create_calendar_simulation_table(sessions, start_date, end_date)
            story.append(calendar_table)
            story.append(Spacer(1, 20))
        
        # Tabla de sesiones
        if sessions:
            story.append(Paragraph(f"Sessions ({start_date.strftime('%d/%m/%Y')} - {end_date.strftime('%d/%m/%Y')})", self.styles['CustomSubtitle']))
            ...
            sessions_table = Table(sessions_data, colWidths=[0.8*inch, 2*inch, 1.5*inch, 1.5*inch, 1.2*inch])
        ...
        if test_results and selected_metrics:
            story.append(Paragraph("Performance Evolution", self.styles['CustomSubtitle']))
            
            # Crear gráfico con matplotlib
            chart_buffer = self._create_test_chart(test_results, selected_metrics, player.user.name)
    ...
    def generate_sessions_report(
        ...
        if sessions:
            ...
            sessions_table = Table(sessions_data, colWidths=[0.6*inch, 1.8*inch, 1.8*inch, 1.2*inch, 1.2*inch, 1*inch])
            sessions_table.setStyle(TableStyle([
                ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#1E88E5')),
                ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
                ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
            ...
            story.append(Paragraph("No sessions found for the selected period.", self.styles['CustomNormal']))
    ...
    def generate_financials_report(
        ...
        # Título
        title = Paragraph(f"Financial Report ({start_date.strftime('%d/%m/%Y')} - {end_date.strftime('%d/%m/%Y')})", self.styles['CustomTitle'])
        story.append(title)
        story.append(Spacer(1, 20))
        ...
        for _, row in df_no_total.iterrows():
            ...
            for i, val in enumerate(row.tolist()):
                ...
                elif 'Ingresos' in df_no_total.columns[i] or 'Gastos' in df_no_total.columns[i]:
        ...
        story.append(Paragraph("Monthly Balance Evolution", self.styles['CustomSubtitle']))
    ...
    def _create_test_chart(self, test_results: List[TestResult], selected_metrics: List[str], player_name: str) -> io.BytesIO:
        ...
        colors_list = ['#1E88E5', '#4CAF50', '#FF9800', '#F44336', '#9C27B0', '#00BCD4', '#8BC34A', '#FFC107', '#795548']
        
        for i, metric in enumerate(selected_metrics):
            if metric in metric_mapping:
                attr_name = metric_mapping[metric]
                values = [getattr(test, attr_name) for test in reversed(test_results) if getattr(test, attr_name) is not None]
                valid_dates = [dates[j] for j, test in enumerate(reversed(test_results)) if getattr(test, attr_name) is not None]
                
                if values:
                    color = colors_list[i % len(colors_list)]
                    # Convert dates to matplotlib dates
                    dates_num = mdates.date2num(valid_dates)
                    plt.plot(dates_num, values, marker='o', label=metric, color=color, linewidth=2, markersize=6)
                    
                    # Format x-axis to show dates properly
                    plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
        
        plt.title(f'Performance Evolution - {player_name}', fontsize=14, fontweight='bold')
    ...
    def _create_calendar_simulation_table(self, sessions: List[Session], start_date: dt.date, end_date: dt.date) -> Table:
        """
    ...
    def _create_day_view_table(self, sessions: List[Session], date: dt.date) -> Table:
        ...
        for hour in hours:
            ...
            for session in day_sessions:
                ...
                session_end = session.end_time.time() if session.end_time else session_start
                
                if session_start <= hour < session_end:
                    session_text = f"{session.coach.user.name} × {session.player.user.name}"
                    break
    ...
    def _create_week_view_table(self, sessions: List[Session], start_date: dt.date, end_date: dt.date) -> Table:
        ...
        for hour in hours:
            ...
            for day in days:
                ...
                for session in sessions:
                    if (session.start_time.date() == day and 
                        session.start_time.time() <= hour < (session.end_time.time() if session.end_time else session.start_time.time())):
        ...
        for week in cal:
            ...
            for day in week:
                if day == 0:
                    ...
                    day_sessions = [s for s in sessions if s.start_time.date() == day_date]
                    if day_sessions:
    ...
    def _create_extended_view_table(self, sessions: List[Session], start_date: dt.date, end_date: dt.date) -> Table:
...
def generate_financials_pdf(start_date: dt.date, end_date: dt.date) -> Tuple[io.BytesIO, str]:
