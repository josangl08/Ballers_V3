...
class ThaiLeagueController:
    ...
    def _find_by_wyscout_id(
        ...
        for player, user in players:
            try:
                player_wyscout_id_int = int(player.wyscout_id) if player.wyscout_id else None
    ...
    def _create_professional_stats_record(
        ...
        for csv_col, db_field in self.COLUMN_MAPPING.items():
            if csv_col in player_data.index:
                ...
                # Convertir números flotantes a enteros donde corresponde
                    ...
                    db_field
                    in [
                        "wyscout_id", "age", "height", "weight", "matches_played", "minutes_played",
                        "goals", "assists", "shots", "yellow_cards", "red_cards", "market_value"
                    ]
        ...
        if not record.get('team') and record.get('team_within_timeframe'):
            ...
            logger.info(f"🔧 Usando team_within_timeframe como fallback para team: {record['team']}")
        ...
        try:
            with self.session_factory() as session:
                ...
                for stat in stats:
                    ...
                    logger.info(f"✅ Estadística procesada: {stat.season} - {stat.team}")
        ...
        try:
            with self.session_factory() as session:
                ...
                for player in professional_players:
                    try:
                        wyscout_id_int = int(player.wyscout_id) if player.wyscout_id else None
                        if wyscout_id_int:
                            wyscout_ids.append(wyscout_id_int)
                    except (ValueError, TypeError):
                        logger.warning(f"WyscoutID inválido: {player.wyscout_id}")
                        
                logger.info(f"Encontrados {len(wyscout_ids)} jugadores profesionales registrados")
                return wyscout_ids
    ...
    def process_season_for_registered_players(
        ...
        """
        try:
            logger.info(f"Procesando temporada {season} para {len(registered_wyscout_ids)} jugadores")
            ...
            if registered_wyscout_ids:
                ...
                df_filtered = df_clean[df_clean['Wyscout id'].isin(registered_wyscout_ids)]
                logger.info(f"Filtrados {len(df_filtered)} de {original_count} registros para jugadores registrados")
            else:
                logger.warning("No hay jugadores profesionales registrados, omitiendo procesamiento")
                return True, f"Temporada {season}: sin jugadores profesionales registrados", {"processed": 0}
            
            if df_filtered.empty:
                return True, f"Temporada {season}: no hay datos para jugadores registrados", {"processed": 0}
    ...
    def load_historical_seasons(
        ...
        if not registered_wyscout_ids:
            ...
            return {
                season: (True, "Sin jugadores profesionales registrados", {"processed": 0})
        ...
        for season in seasons:
            ...
            try:
                ...
                with self.session_factory() as session:
                    season_record = session.query(ThaiLeagueSeason).filter_by(season=season).first()
                    
                    if season_record and season_record.import_status == ImportStatus.completed:
                        logger.info(f"Temporada {season} ya está completada, omitiendo")
                        results[season] = (True, f"Temporada {season} ya importada", {"skipped": True})
                        continue
        ...
        logger.info(f"Carga histórica completada. Resultados: {len([r for r in results.values() if r[0]])} exitosas")
        return results

    def trigger_stats_import_for_player(self, player_id: int, wyscout_id: int) -> Tuple[bool, str, Dict]:
        """
        ...
        Returns:
            Tuple[success, message, statistics]
        """
        logger.info(f"🎯 Trigger automático: importando estadísticas para player_id={player_id}, wyscout_id={wyscout_id}")
        ...
        try:
            ...
            for season in available_seasons:
                try:
                    ...
                    logger.info(f"🔍 Procesando temporada {season} para WyscoutID {wyscout_id_int}")
                    
                    # Verificar si ya existe el registro para evitar duplicados
                    with self.session_factory() as session:
                        existing = session.query(ProfessionalStats).filter_by(
                            player_id=player_id,
                            season=season
                        ).first()
                        
                        if existing:
                            logger.info(f"⏭️ Registro ya existe para {season}, omitiendo")
                    ...
                    if not success or season_data is None or season_data.empty:
                        logger.warning(f"⚠️ No se pudieron cargar datos para temporada {season}: {message}")
                        continue
                    ...
                    if player_rows.empty:
                        logger.info(f"🔍 WyscoutID {wyscout_id_int} no encontrado en temporada {season}")
                        continue
                    ...
                    for _, player_data in player_rows.iterrows():
                        try:
                            ...
                            with self.session_factory() as session:
                                ...
                                logger.info(f"✅ Estadísticas importadas para temporada {season}")
                                
                        except Exception as e:
                            stats["errors"] += 1
                            logger.error(f"❌ Error importando registro de temporada {season}: {str(e)}")
    ...
    def sync_season_status_from_real_data(self) -> Tuple[bool, str, Dict]:
        ...
        try:
            ...
            with self.session_factory() as session:
                ...
                real_stats = session.query(
                    ProfessionalStats.season,
                    func.count(ProfessionalStats.stat_id).label('real_count'),
                    func.count(func.distinct(ProfessionalStats.wyscout_id)).label('unique_players')
                ).group_by(ProfessionalStats.season).all()
                ...
                logger.info(f"📊 Datos reales encontrados: {len(real_data_by_season)} temporadas")
                ...
                for season_record in season_records:
                    ...
                    real_data = real_data_by_season.get(season_name, {"count": 0, "players": 0})
                    ...
                    if real_count > 0 and season_record.import_status == ImportStatus.in_progress:
                    ...
                    if season_record.imported_records != real_count:
                        ...
                        changes_made.append(f"imported_records: {old_count} → {real_count}")
                        was_discrepancy = True
                    ...
                    if season_record.matched_players != real_players:
                        ...
                        changes_made.append(f"matched_players: {old_players} → {real_players}")
                        was_discrepancy = True
                    ...
                    logger.info(f"  📋 {season_name}: {real_count} registros, {len(changes_made)} cambios")
            ...
            with self.session_factory() as session:
                ...
                for season_record in season_records:
                    ...
                    if season_name in self.AVAILABLE_SEASONS:
                        ...
                        if not season_record.source_url or season_record.source_url != source_url:
                            ...
                            url_results["details"][season_name] = {
                                "old_url": old_url,
                                "new_url": source_url,
                                "action": "generated" if old_url == "N/A" else "updated"
                            }
                            
                            logger.info(f"  🔗 {season_name}: URL {'generada' if old_url == 'N/A' else 'actualizada'}")
                    else:
                        logger.warning(f"  ⚠️ {season_name}: no está en AVAILABLE_SEASONS")
                
                session.commit()
                
                total_changes = url_results["urls_generated"] + url_results["urls_updated"]
        ...
        try:
            ...
            if cache_file.exists():
                ...
                try:
                    ...
                    logger.info(f"  📁 Archivo cache: {data['file_size']} bytes, {data['total_records']} registros")
                except Exception as e:
            ...
            with self.session_factory() as session:
                ...
                real_stats = session.query(
                    ...
                    func.count(func.distinct(ProfessionalStats.wyscout_id)).label('unique_players')
                ).filter(ProfessionalStats.season == season).first()
                
                if real_stats:
                    data["matched_players"] = real_stats.unique_players or 0
                    logger.info(f"  👥 Jugadores coincidentes: {data['matched_players']}")
                
                # 3. Calcular unmatched (total - matched)
                if data["total_records"] > 0 and data["matched_players"] > 0:
                    data["unmatched_players"] = data["total_records"] - data["matched_players"]
                    logger.info(f"  ❌ Jugadores no coincidentes: {data['unmatched_players']}")
    ...
    def generate_import_logs(self, season: str, stats: Dict, errors: List = None) -> Tuple[str, str]:
        """
    ...
    def configure_completion_settings(self, season: str, is_completed: bool) -> Dict:
        ...
        """
        logger.info(f"⚙️ Configurando settings para {season} (completed: {is_completed})")
        ...
        logger.debug(f"  📅 {season}: año_inicio={season_start_year}, histórica={is_historical}")
        ...
        try:
            ...
            with self.session_factory() as session:
                ...
                for season_record in season_records:
                    ...
                    desired_auto_update = not is_historical  # False para históricas, True para actuales
                    
                    # También configurar la frecuencia según el tipo
                    desired_frequency = 0 if is_historical else 7  # 0 para históricas, 7 para actuales
                    ...
                    if season_record.auto_update_enabled != desired_auto_update:
                        ...
                        changes_made.append(f"auto_update: {old_setting} → {desired_auto_update}")
                    
                    if season_record.update_frequency_days != desired_frequency:
                        season_record.update_frequency_days = desired_frequency
                        changes_made.append(f"frequency: {old_frequency} → {desired_frequency}")
                    ...
                    if changes_made:
                        ...
                        config_results["details"][season_name] = {
                            "season_type": "historical" if is_historical else "current/future",
                            "changes_made": changes_made,
                        ...
                        logger.info(f"  ⚙️ {season_name}: {action} ({', '.join(changes_made)})")
                    else:
        ...
        try:
            ...
            with self.session_factory() as session:
                ...
                if cache_file.exists():
                    ...
                    current_dt = self._get_current_datetime()
                    file_age_hours = (current_dt.timestamp() - cache_file.stat().st_mtime) / 3600
                    
                    update_needed = (
                        season_record.import_status != ImportStatus.completed or
                        file_age_hours > 168  # Más de una semana
                    )
                    
                    return {
                        "season": season,
                        "update_available": update_needed,
                        "reason": "Verificación periódica" if update_needed else "Cache actual",
                        "local_hash": local_hash,
                        "file_age_hours": round(file_age_hours, 1),
                        "last_import": season_record.last_import_attempt.isoformat() if season_record.last_import_attempt else None
                    }
                else:
                    return {
                        "season": season,
    ...
    def perform_incremental_update(self, season: str = None) -> Tuple[bool, str, Dict]:
        ...
        try:
            ...
            if not update_info.get("update_available", False):
                return True, f"Temporada {update_info['season']}: no requiere actualización", {
            ...
            if not registered_wyscout_ids:
                return True, f"Temporada {season}: sin jugadores profesionales para actualizar", {
                    "season": season,
                    "updated": False,
            ...
            if success:
                ...
                return True, f"Temporada {season} actualizada exitosamente", {
                    "season": season,
                    "updated": True,
                    **stats
                }
            else:
                logger.error(f"❌ Error en actualización incremental de {season}: {message}")
    ...
    def auto_update_all_seasons(self) -> Dict[str, Tuple[bool, str, Dict]]:
        ...
        seasons_to_check = list(self.AVAILABLE_SEASONS.keys())[-3:]  # Últimas 3 temporadas
        ...
        successful_updates = len([r for r in results.values() if r[0] and r[2].get('updated', False)])
        logger.info(f"Actualización automática completada: {successful_updates} temporadas actualizadas")
    ...
    def get_last_csv_update_date(self, season: str = None) -> datetime:
        ...
        try:
            ...
            logger.error(f"Error obteniendo fecha de actualización para {season}: {str(e)}")
            return datetime(2020, 1, 1)

    def csv_updated_recently(self, season: str = None, days_threshold: int = 30) -> bool:
        """
        ...
        try:
            ...
            logger.debug(f"CSV {season}: última actualización hace {days_since_update} días")
    ...
    def determine_season_action(self) -> str:
        ...
        try:
            if current_month in [7, 8]:
                ...
                logger.info(f"Mes {current_month}: Período de búsqueda de nueva temporada")
                ...
                if self.csv_updated_recently(days_threshold=30):
                    ...
                    logger.info(f"Mes {current_month}: Temporada inactiva por >30 días, marcando como terminada")
                    return 'season_ended'
                    
            elif current_month == 6:
                # Junio: Período muerto
                logger.info(f"Mes {current_month}: Período de descanso entre temporadas")
                return 'season_ended'
            ...
            for season in possible_seasons:
                if season in self.AVAILABLE_SEASONS:
                    ...
                    with self.session_factory() as session:
                        ...
                        if not season_record or season_record.import_status != ImportStatus.completed:
                    ...
                    try:
                        ...
                        if response.status_code == 200:
                            logger.info(f"🆕 Nueva temporada descubierta en repositorio: {season}")
                            # Añadir a temporadas disponibles
                            self.AVAILABLE_SEASONS[season] = filename
                            return True, f"Nueva temporada descubierta: {season}", season
        ...
        try:
            ...
            if action == 'update_current':
                ...
                if found and new_season:
                    ...
                    if registered_ids:
                        success, import_message, stats = self.process_season_for_registered_players(
    ...
    def sync_all_seasons_data(self) -> Tuple[bool, str, Dict]:
        ...
        try:
            ...
            success_3, message_3, details_3 = self.configure_auto_update_by_season_type()
            ...
            master_results["summary"] = {
                ...
                "historical_seasons_disabled": details_3.get("historical_seasons_disabled", 0),
    ...
    def sync_all_comprehensive_data(self) -> Tuple[bool, str, Dict]:
        ...
        try:
            ...
            logger.info("📁 Paso 2/4: Recopilando datos completos (file_size, total_records, etc.)")
            ...
            with self.session_factory() as session:
                ...
                for season_record in season_records:
                    ...
                    comprehensive_data = self.collect_comprehensive_season_data(season_name)
                    
                    # Variables para tracking de cambios
                    changes_made = []
                    
                    # Actualizar file_size
                    if comprehensive_data["file_size"] and season_record.file_size != comprehensive_data["file_size"]:
                        old_size = season_record.file_size or 0
                        season_record.file_size = comprehensive_data["file_size"]
                        changes_made.append(f"file_size: {old_size} → {comprehensive_data['file_size']}")
                        comprehensive_sync_results["total_file_size"] += comprehensive_data["file_size"]
                    
                    # Actualizar total_records
                    if season_record.total_records != comprehensive_data["total_records"]:
                        old_total = season_record.total_records or 0
                        season_record.total_records = comprehensive_data["total_records"]
                        changes_made.append(f"total_records: {old_total} → {comprehensive_data['total_records']}")
                        comprehensive_sync_results["total_csv_records"] += comprehensive_data["total_records"]
                    
                    # Actualizar unmatched_players
                    if season_record.unmatched_players != comprehensive_data["unmatched_players"]:
                        old_unmatched = season_record.unmatched_players or 0
                        season_record.unmatched_players = comprehensive_data["unmatched_players"]
                        changes_made.append(f"unmatched_players: {old_unmatched} → {comprehensive_data['unmatched_players']}")
                    ...
                    if comprehensive_data["errors_count"] > 0:
                        ...
                        changes_made.append(f"errors_count: → {comprehensive_data['errors_count']}")
                    
                    # Generar y actualizar logs
                    import_log, error_log = self.generate_import_logs(season_name, comprehensive_data)
                    
                    if not season_record.import_log or len(season_record.import_log) < 10:
                    ...
                    completion_config = self.configure_completion_settings(season_name, is_completed)
                    ...
                    for config_key, config_value in completion_config.items():
                        if hasattr(season_record, config_key):
                            ...
                            if old_value != config_value:
                                ...
                                changes_made.append(f"{config_key}: {old_value} → {config_value}")
            ...
            comprehensive_results["comprehensive_summary"] = {
                ...
                "total_file_size_mb": round(comprehensive_sync_results["total_file_size"] / (1024 * 1024), 2),
            ...
            return comprehensive_results["overall_success"], final_message, comprehensive_results
    ...
    def _validate_comprehensive_sync(self) -> Dict:
        ...
        try:
            with self.session_factory() as session:
                ...
                logger.info(f"📊 Validación: {validation_results['valid_seasons']}/{validation_results['total_seasons']} temporadas válidas")
    ...
    def apply_seasonal_logic(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"🗓️ Aplicando lógica estacional Thai League - Mes actual: {current_month}")
            
            # Mayo-Junio: Marcar temporadas como completadas
            if current_month in [5, 6]:  # Mayo y Junio
                logger.info("📅 Periodo de finalización (Mayo-Junio): evaluando temporadas")
                completed_seasons = self._mark_seasons_completed_if_stale()
                results["season_completed"] = completed_seasons
                results["actions_taken"].append(f"Evaluated season completion (month {current_month})")
            
            # Julio-Agosto: Buscar nueva temporada
            elif current_month in [7, 8]:  # Julio y Agosto
                logger.info("🔍 Periodo de búsqueda (Julio-Agosto): buscando nueva temporada")
                new_season = self._search_for_new_season()
                results["new_season_found"] = new_season
                results["actions_taken"].append(f"Searched for new season (month {current_month})")
            
            else:
                logger.info(f"⏸️ Mes {current_month}: sin acciones estacionales específicas")
                results["actions_taken"].append(f"No seasonal actions for month {current_month}")
    ...
    def _mark_seasons_completed_if_stale(self) -> List[str]:
        ...
        try:
            with self.session_factory() as session:
                ...
                seasons = session.query(ThaiLeagueSeason).filter(
                    ThaiLeagueSeason.import_status.in_([ImportStatus.in_progress, ImportStatus.updated, ImportStatus.completed]),
                ...
                for season in seasons:
                    ...
                    if season.season in ["24-25", "2024-25"] and current_year == 2025:
                        if current_month >= 7:  # Julio o después - temporada terminó hace >60 días
                    ...
                    if should_mark_completed:
                        ...
                        logger.info(f"✅ Temporada {season.season} marcada como completada: {reason}")
                
                if completed_seasons:
                    session.commit()
                    logger.info(f"💾 Guardadas {len(completed_seasons)} temporadas como completadas")
    ...
    def _search_for_new_season(self) -> Optional[str]:
        ...
        try:
            ...
            with self.session_factory() as session:
                existing = session.query(ThaiLeagueSeason).filter(
                    ThaiLeagueSeason.season == next_season
                ).first()
            ...
            if success and df is not None:
                ...
                with self.session_factory() as session:
                    ...
                    logger.info(f"🎉 Nueva temporada {next_season} detectada y registrada")
                    return next_season
            else:
    ...
    def generate_contextual_notes(self, season: str) -> str:
        ...
        try:
            ...
            if season_year < current_year - 1:
                ...
                if current_month >= 7:  # Julio o después, temporada definitivamente terminada
    ...
    def sync_seasonal_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("🔧 Aplicando correcciones específicas a temporada 2024-25 (paso final)")
            ...
            if corrections_2024_result.get("corrections_applied"):
                summary_parts.append(f"2024-25 corrections: {len(corrections_2024_result['corrections_applied'])}")
            if seasonal_results.get("season_completed"):
                summary_parts.append(f"{len(seasonal_results['season_completed'])} seasons completed")
            if seasonal_results.get("new_season_found"):
                summary_parts.append(f"new season {seasonal_results['new_season_found']} detected")
            if file_sizes_updated > 0:
                summary_parts.append(f"{file_sizes_updated} file sizes updated")
            if notes_updated > 0:
                summary_parts.append(f"{notes_updated} notes updated")
            
            if summary_parts:
                results["message"] = f"Seasonal sync completed: {', '.join(summary_parts)}"
            else:
                results["message"] = "Seasonal sync completed: no changes required"
            
            logger.info(f"✅ Sincronización estacional completada: {results['message']}")
            return results
            ...
            with self.session_factory() as session:
                ...
                for season in seasons:
                    ...
                    if season.notes != new_note:
                        ...
                        updated_count += 1
                        logger.debug(f"Actualizada nota para {season.season}: '{new_note}'")
    ...
    def apply_2024_25_corrections(self) -> Dict[str, Any]:
        ...
        try:
            ...
            with self.session_factory() as session:
                ...
                season = session.query(ThaiLeagueSeason).filter(
                    ThaiLeagueSeason.season.in_(["24-25", "2024-25"])
                ).first()
                
                if not season:
                    return {"success": False, "message": "Temporada 2024-25 no encontrada"}
                
                logger.info(f"🔧 Aplicando correcciones específicas a temporada {season.season}")
                ...
                if corrections_applied:
                    session.commit()
                    logger.info(f"✅ Aplicadas {len(corrections_applied)} correcciones a temporada {season.season}")
                    for correction in corrections_applied:
                        logger.info(f"   - {correction}")
                else:
                    logger.info(f"ℹ️ Temporada {season.season} ya tiene configuración correcta")
                
                return {
                    "success": True,
                    "season": season.season,
                    "corrections_applied": corrections_applied,
                    "message": f"Temporada {season.season}: {len(corrections_applied)} correcciones aplicadas" if corrections_applied else f"Temporada {season.season} ya configurada correctamente"
                }
    ...
    def smart_weekly_update(self) -> Dict[str, Any]:
        ...
        """
        try:
            logger.info("🔄 Ejecutando actualización semanal inteligente con lógica estacional")
            ...
            if results.get("success", True):
                return {
                    ...
                    "stats": {
                        "seasonal_actions": len(results.get("seasonal_logic", {}).get("actions_taken", [])),
                        "file_sizes_updated": results.get("file_sizes_updated", 0),
                        "notes_updated": results.get("notes_updated", 0),
                        "seasons_completed": len(results.get("seasonal_logic", {}).get("season_completed", [])),
                        "new_season_found": results.get("seasonal_logic", {}).get("new_season_found") is not None
                    }
                }
            else:
                return {
    ...
    def get_team_info(self, stats_record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "has_transfer": bool(team_current != team_start and team_current and team_start)
        }
    ...
    def process_team_data_with_intelligence(self, stats_record: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if 'team_within_timeframe' not in stats_record or not stats_record['team_within_timeframe']:
    ...
    def process_season_with_modular_etl(
        ...
        """
        logger.info(f"🚀 Procesando {season} con pipeline ETL modular (threshold={threshold})")
        ...
        try:
            ...
            error_msg = f"Error inesperado en pipeline ETL modular para {season}: {str(e)}"
            logger.error(error_msg)
