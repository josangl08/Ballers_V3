...
class StatsAnalyzer:
    ...
    def analyze_dataframe(self, df: pd.DataFrame, season: str) -> Dict[str, any]:
        """
    ...
    def _analyze_data_quality(self, df: pd.DataFrame) -> Dict[str, any]:
        ...
        if "Age" in df.columns:
            ...
            if total_ages > 0:
                quality_metrics["validity"]["age_validity_pct"] = round(
                    (valid_ages / total_ages) * 100, 2
                )
    ...
    def _analyze_team_stats(self, df: pd.DataFrame) -> Dict[str, any]:
        ...
        if "Team" in df.columns:
            ...
            if len(team_df) > 0:
                ...
                team_stats["top_teams_by_players"] = (
                    players_per_team.sort_values(ascending=False).head(5).to_dict()
                )
        ...
        if "Team" in df.columns:
            ...
            team_stats["players_without_team_pct"] = round(
                (players_without_team / len(df)) * 100, 2
            )
    ...
    def _analyze_performance_insights(self, df: pd.DataFrame) -> Dict[str, any]:
        ...
        if "Goals" in df.columns and "Shots" in df.columns:
            ...
            efficiency_df = df[
                (df["Shots"] >= 5) & (df["Goals"].notna()) & (df["Shots"].notna())
            ].copy()
            if len(efficiency_df) > 0:
                efficiency_df["goal_efficiency"] = (
                    efficiency_df["Goals"] / efficiency_df["Shots"]
                )
    ...
    def _generate_summary(self, analysis: Dict[str, any]) -> Dict[str, any]:
        ...
        summary["quality_assessment"] = {
            ...
            "players_without_team": analysis["team_stats"].get(
        ...
        if "top_scorers" in analysis["performance_insights"]:
            ...
            insights.append(
                f"Máximo goleador: {top_scorer['Full name']} con {top_scorer['Goals']} goles"
            )

        if analysis["team_stats"].get("players_without_team", 0) > 0:
            no_team_count = analysis["team_stats"]["players_without_team"]
            insights.append(
                f"Jugadores sin equipo: {no_team_count} (requiere revisión)"
            )
    ...
    def generate_matching_report(
        ...
        report["matching_summary"] = {
            ...
            "success_rate_pct": (
                round(((exact_count + fuzzy_count) / total_count * 100), 1)
                if total_count > 0
                else 0
            ),
        }
        ...
        if no_match_count > 0:
            recommendations.append(
                f"Revisar {no_match_count} jugadores sin matching - posibles nuevos profesionales"
            )

        if multiple_count > 0:
            recommendations.append(
                f"Resolver {multiple_count} matchings múltiples manualmente"
            )

        if fuzzy_count > exact_count:
            recommendations.append(
                "Alto número de fuzzy matches - considerar mejorar datos de referencia"
            )
