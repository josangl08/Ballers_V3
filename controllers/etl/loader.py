...
class ThaiLeagueLoader:
    ...
    def import_season_data(
        self, season: str, df: pd.DataFrame, matching_results: Dict, column_mapping: Dict
    ) -> Tuple[bool, str, Dict]:
        """
        ...
        try:
                ...
                for i, match in enumerate(all_matches):
                    try:
                        ...
                        if player_rows.empty:
                            stats["error_details"].append(f"Jugador no encontrado en CSV: {match['csv_player']}")
                            stats["errors"] += 1
                            continue
                        ...
                        prof_stats = self._create_professional_stats_record(
                            player_data, season, match["matched_player"]["player_id"], column_mapping
                        )
                        ...
                        existing = (
                            ...
                            .filter(
                                ProfessionalStats.player_id == match["matched_player"]["player_id"],
                        ...
                        if existing:
                            ...
                            logger.debug(f"Actualizado registro existente para {match['csv_player']}")
                        else:
                            # Crear nuevo registro
                            new_stats = ProfessionalStats(**prof_stats)
                            session.add(new_stats)
                            logger.debug(f"Creado nuevo registro para {match['csv_player']}")
            ...
            try:
                ...
                logger.error(f"Error adicional marcando temporada como fallida: {inner_e}")
    ...
    def _create_professional_stats_record(
        ...
        problematic_fields = [field for field in prof_stats.keys() if field.endswith('_processed')]
            ...
            try:
                ...
                for j, record in enumerate(batch):
                    try:
                        ...
                        session.rollback()
                        individual_error = f"Error en registro individual {i + j}: {str(individual_e)}"
                        error_details.append(individual_error)
                        logger.error(individual_error)
        
        logger.info(f"📊 Inserción completa: {inserted_count} exitosos, {error_count} errores")
        ...
        try:
            with self.session_factory() as session:
                ...
                if season_obj:
                    ...
                    logger.warning(f"⚠️ Temporada {season} no encontrada para actualizar metadatos")
                    return False
