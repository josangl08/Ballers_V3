...
def get_accounting_df():
    ...
    try:
        ...
        if len(data) < 2:
            st.warning("⚠️ Google Sheets no tiene suficientes datos (necesita al menos encabezados + 1 fila)")
            return pd.DataFrame()
        ...
        st.info("2. Verificar que la cuenta de servicio tenga permisos de lectura en el Sheet")
