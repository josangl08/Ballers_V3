...
class WebhookIntegration:
    ...
    def _register_google_calendar_channel(self) -> bool:
        ...
        try:
            ...
            if is_local_dev:
                ...
                logger.info("💡 For full webhook testing, consider using ngrok for HTTPS")
            else:
            ...
            if self.webhook_channel:
                if is_local_dev:
                    ...
                    logger.info("✅ Google Calendar will send real-time notifications to localhost")
                else:
                    logger.info("✅ Production webhook channel registered successfully!")
                ...
                if is_local_dev:
                    logger.warning("⚠️ Could not register webhook in development environment")
                    logger.info("🔄 Webhook server running for manual testing")
                    logger.info("🛠️ You can test webhook endpoint manually:")
                    logger.info(f"   curl -X POST {webhook_url} -H 'Content-Type: application/json'")
            ...
            if is_local_dev:
                ...
                logger.error(f"❌ Critical error registering webhook in production: {e}")
                return False
    ...
    def _start_auto_renewal_thread(self):
        ...
        try:
            ...
            self.auto_renewal_thread = threading.Thread(
                target=self._auto_renewal_loop, 
                daemon=True
            )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            is_development = endpoint_url and ("localhost" in endpoint_url or "127.0.0.1" in endpoint_url)
            
            # Estado del channel de Google Calendar
            channel_status = get_webhook_channel_status()
            has_real_channel = self.webhook_channel and not self.webhook_channel.get("development_mode", False)
            
            # Estado del thread de auto-renewal
            auto_renewal_active = (
                self.auto_renewal_thread and 
                self.auto_renewal_thread.is_alive() and 
                not self._stop_renewal
            )
            ...
            if is_development:
                ...
                integration_healthy = (
                    server_running and 
                    channel_status.get("active", False) and 
                    auto_renewal_active
                )
                capabilities = ["real_time_sync", "google_calendar_notifications", "auto_renewal"]
            ...
            return {
                ...
                "google_calendar_channel": {
                    **channel_status,
                    ...
                    "development_mode": self.webhook_channel.get("development_mode", False) if self.webhook_channel else False
                },
                "auto_renewal": {
                    "active": auto_renewal_active,
                    "enabled": not is_development,
                    "thread_alive": self.auto_renewal_thread.is_alive() if self.auto_renewal_thread else False
                },
