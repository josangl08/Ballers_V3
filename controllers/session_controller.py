...
class SessionController:
    ...
    def create_session(
        ...
        try:
            ...
            coach_name_snapshot = (
                coach.user.name if coach and coach.user else f"Coach {coach_id}"
            )
            player_name_snapshot = (
                player.user.name if player and player.user else f"Player {player_id}"
            )
            ...
            if coach_changed or player_changed:
                ...
                if player_changed and hasattr(session, "player_id"):
                    player = (
                        self.db.query(Player)
                        .options(joinedload(Player.user))
                        .filter_by(player_id=session.player_id)
                        .first()
                    )
                    session.player_name_snapshot = (
                        player.user.name
                        if player and player.user
                        else f"Player {session.player_id}"
                    )
