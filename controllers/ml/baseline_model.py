...
from sklearn.model_selection import (
    cross_val_score, KFold, StratifiedKFold, learning_curve,
...
class BaselineModel(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, model_name: str = "BaselineModel"):
        ...
        self.baseline_features = [
            # Básicas (SIN goals/assists directos)
            'matches_played', 'minutes_played', 'age',
            ...
            'defensive_duels_won_pct', 'interceptions_per_90', 'tackles_per_90', 'clearances_per_90',
            # Métricas ofensivas per_90 (SIN goals/assists directos)
            'shots_per_90', 'shots_on_target_pct',
            # Features derivadas NO circulares
            'shot_efficiency', 'pass_quality_score', 'participation_ratio',
            # Disciplina
            'yellow_cards', 'red_cards'
        ]
        
        logger.info(f"Inicializado {self.model_name} con {len(self.baseline_features)} features base")
    ...
    def map_csv_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(f"Columnas mapeadas: {len([c for c in self.csv_column_mapping.keys() if c in df.columns])}/{len(self.csv_column_mapping)}")
        return mapped_df
    ...
    def extract_baseline_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            available_features = [f for f in self.baseline_features if f in mapped_df.columns]
            
            logger.info(f"Features disponibles: {len(available_features)}/{len(self.baseline_features)}")
            
            # Extraer features básicas
            position_col = 'primary_position' if 'primary_position' in mapped_df.columns else 'Primary position'
            base_columns = available_features + [position_col] if position_col in mapped_df.columns else available_features
            ...
            if 'goals' in feature_df.columns and 'minutes_played' in feature_df.columns:
                ...
                if 'goals_per_90' not in feature_df.columns:
                    feature_df['goals_per_90_calc'] = (
                        feature_df['goals'].fillna(0) * 90 / feature_df['minutes_played'].fillna(90)
                    ).clip(0, 10)  # Clip valores extremos
                
            if 'assists' in feature_df.columns and 'minutes_played' in feature_df.columns:
                if 'assists_per_90' not in feature_df.columns:
                    feature_df['assists_per_90_calc'] = (
                        feature_df['assists'].fillna(0) * 90 / feature_df['minutes_played'].fillna(90)
                    ).clip(0, 5)  # Clip valores extremos
                
            # Feature de participación (ratio de minutos jugados)
            if 'minutes_played' in feature_df.columns and 'matches_played' in feature_df.columns:
                feature_df['participation_ratio'] = (
                    feature_df['minutes_played'].fillna(0) / (feature_df['matches_played'].fillna(1) * 90)
                ).clip(0, 1)
            ...
            if 'shots_per_90' in feature_df.columns and 'shots_on_target_pct' in feature_df.columns:
                feature_df['shot_efficiency'] = (
                    feature_df['shots_per_90'].fillna(0) * 
                    feature_df['shots_on_target_pct'].fillna(0) / 100
                ).clip(0, 10)
            
            # Pass quality score (proxy para creatividad)
            if 'passes_per_90' in feature_df.columns and 'pass_accuracy_pct' in feature_df.columns:
                feature_df['pass_quality_score'] = (
                    feature_df['passes_per_90'].fillna(0) * 
                    feature_df['pass_accuracy_pct'].fillna(0) / 100
                ).clip(0, 200)
            
            # Encoding simple de posiciones (One-Hot)
            if position_col in feature_df.columns:
                position_dummies = pd.get_dummies(
                    feature_df[position_col], 
                    prefix='position',
                    dummy_na=True
                )
    ...
    def calculate_target_pdi(self, df: pd.DataFrame) -> np.ndarray:
        ...
        try:
            ...
            for _, row in mapped_df.iterrows():
                ...
                zone_score = 50.0
                position = row.get('primary_position') or row.get('Primary position', 'CMF')
                
                if position in ['CF', 'W', 'AMF', 'LWF', 'RWF', 'LAMF', 'RAMF']:  # Ofensivos
                ...
                elif position in ['DMF', 'CB', 'GK', 'RCB', 'LCB', 'RDMF', 'LDMF']:  # Defensivos
                    def_duels = row.get('defensive_duels_won_pct') or row.get('Defensive duels won, %')
                    if pd.notna(def_duels):
                        zone_score += (def_duels - 50) * 0.4
                    interceptions = row.get('interceptions_per_90') or row.get('Interceptions per 90')
            ...
            logger.info(f"PDI stats: mean={np.mean(pdi_scores):.1f}, std={np.std(pdi_scores):.1f}")
...
class LinearBaselineModel(BaselineModel):
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, 
            feature_names: Optional[List[str]] = None) -> 'LinearBaselineModel':
        """
...
class RidgeBaselineModel(BaselineModel):
    ...
    def fit(self, X: np.ndarray, y: np.ndarray,
            feature_names: Optional[List[str]] = None) -> 'RidgeBaselineModel':
        ...
        try:
            ...
            logger.info(f"Ridge Baseline (α={self.alpha}) entrenado en {self.training_time:.2f}s")
            logger.info(f"R² entrenamiento: {self.model.score(X_scaled, y):.3f}")
    ...
    def get_feature_importance(self) -> Dict[str, float]:
        ...
        coefficients = np.abs(self.model.coef_)
        importance = coefficients / np.sum(coefficients) if np.sum(coefficients) > 0 else coefficients
...
class EnsembleBaselineModel(BaselineModel):
    ...
    def fit(self, X: np.ndarray, y: np.ndarray,
            feature_names: Optional[List[str]] = None) -> 'EnsembleBaselineModel':
        ...
        try:
            ...
            logger.info(f"R² componentes - Linear: {linear_r2:.3f}, Ridge: {ridge_r2:.3f}, RF: {rf_r2:.3f}")
    ...
    def get_feature_importance(self) -> Dict[str, float]:
        ...
        linear_norm = linear_coef / np.sum(linear_coef) if np.sum(linear_coef) > 0 else linear_coef
        ridge_norm = ridge_coef / np.sum(ridge_coef) if np.sum(ridge_coef) > 0 else ridge_coef
        rf_norm = rf_importance / np.sum(rf_importance) if np.sum(rf_importance) > 0 else rf_importance
...
class BaselineEvaluator:
    ...
    def evaluate_model(self, model: BaselineModel, X: np.ndarray, y: np.ndarray,
                      positions: Optional[np.ndarray] = None) -> BaselineResults:
        """
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"No se pudo obtener importancia de features para {model.model_name}")
    ...
    def plot_model_comparison(self, results: List[BaselineResults]) -> None:
        ...
        print(f"   🥇 Más Estable: {most_stable['Model']} (Estabilidad = {most_stable['CV_Stability']:.3f})")
    
    def generate_learning_curves(self, model: BaselineModel, X: np.ndarray, y: np.ndarray) -> None:
        """
...
def run_comprehensive_baseline_evaluation(seasons: List[str] = None) -> Dict[str, BaselineResults]:
    """
    ...
    try:
        ...
        print(f"   📈 MAE: {best_overall['MAE']:.3f} ± {best_overall['CV_MAE_std']:.3f}")
        ...
        print("🚀 RECOMENDACIÓN: Proceder con modelos avanzados basados en estos resultados")
        print("="*80)
