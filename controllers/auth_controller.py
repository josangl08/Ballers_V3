...
class AuthController(LoggerMixin):
    ...
    def create_session(self, user: User, remember_me: bool = False) -> Dict[str, Any]:
        ...
        if remember_me:
            st.query_params.update({"auto_login": "true", "uid": str(user.user_id)})
    ...
    def clear_session(self, show_message: bool = True) -> None:
        ...
        keys_to_delete = [
            key for key in st.session_state.keys() if key != "just_logged_out"
        ]
    ...
    def require_user_type(self, required_types: list) -> bool:
        ...
        if not self.check_user_type(required_types):
            ...
            st.error(
                f"Access denied. Required: {', '.join(required_types)}. Current: {current_type}"
            )
            st.stop()
