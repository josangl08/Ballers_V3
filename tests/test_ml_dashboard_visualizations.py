...
class TestPDIEvolutionChart:
    ...
    @patch('pages.ballers_dash.MLMetricsController')
    def test_pdi_evolution_chart_creation_success(self, mock_controller_class):
        ...
        mock_controller.get_enhanced_player_analysis.side_effect = [
            mock_analysis_2023, mock_analysis_2024
        ]

        # ACT
        result = create_pdi_evolution_chart(player_id=123, seasons=["2023-24", "2024-25"])
    ...
    @patch('pages.ballers_dash.MLMetricsController')
    def test_pdi_evolution_chart_validates_figure_structure(self, mock_controller_class):
...
class TestMLEnhancedRadarChart:
    ...
    def test_ml_radar_chart_creation_success(self, mock_normalizer_class, mock_controller_class):
        ...
        mock_controller.get_enhanced_player_analysis.assert_called_once_with(123, "2024-25")

    @patch('pages.ballers_dash.MLMetricsController')
    @patch('pages.ballers_dash.PositionNormalizer')
    def test_ml_radar_chart_validates_scatterpolar_structure(self, mock_normalizer_class, mock_controller_class):
        ...
        expected_params = [
            'Technical Skills', 'Tactical Intelligence', 'Physical Performance',
            'Consistency', 'Universal Skills', 'Zone Performance', 'Position Specific'
        ]
    ...
    @patch('pages.ballers_dash.MLMetricsController')
    @patch('pages.ballers_dash.PositionNormalizer')
    def test_ml_radar_chart_with_default_season(self, mock_normalizer_class, mock_controller_class):
    ...
    @patch('pages.ballers_dash.MLMetricsController')
    @patch('pages.ballers_dash.PositionNormalizer')
    def test_ml_radar_chart_handles_errors(self, mock_normalizer_class, mock_controller_class):
        ...
        mock_controller.__enter__ = Mock(return_value=mock_controller)
        mock_controller.__exit__ = Mock(return_value=None)
        mock_controller.get_enhanced_player_analysis.side_effect = Exception("Data Error")
...
class TestProfessionalStatsContentIntegration:
    ...
    def test_professional_stats_content_structure(self):
        ...
        # ACT
        with patch('pages.ballers_dash.create_pdi_evolution_chart') as mock_pdi, \
...
class TestMLVisualizationErrorHandling:
    ...
    def test_pdi_chart_invalid_player_id(self):
        ...
        assert result is not None
        assert isinstance(result, type(dbc.Alert()))
        assert "No ML metrics available" in str(result.children) or "Error" in str(result.children)
...
class TestMLVisualizationPerformance:
    ...
    def test_pdi_chart_response_time(self, mock_controller_class):
        ...
        assert execution_time < 1.0, f"Chart creation took {execution_time:.3f}s, too slow"

    @patch('pages.ballers_dash.MLMetricsController')
    @patch('pages.ballers_dash.PositionNormalizer')
    def test_ml_radar_chart_memory_usage(self, mock_normalizer_class, mock_controller_class):
